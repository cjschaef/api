...
var map_IBMCloudPlatformStatus = map[string]string{
	...
	"networkResourceGroupName": "NetworkResourceGroupName is the Resource Group for network resources like the VPC and Subnets used by the cluster, where ResourceGroupName will contain the remaining resources (e.g., VSI's). If empty, the NetworkResourceGroupName is considered to be the same value as ResourceGroupName, which will contain the network and remaining resources of the cluster. This field is immutable and cannot be changed after cluster installation.",
